...
public class BeginController {
	 ...
	 */
	private ArrayList<Integer> cubeNumberList = new ArrayList<>();
	...
	private ArrayList<String> rivalList = new ArrayList<>();
	 ...
	 */
	void initialize(ArrayList<String> playerName, ArrayList<String> color, ArrayList<String> rival, int numberOfPlayers) {
	 ...
	 */
	private void beginningRollTheDice(int numberOfPlayers) {
	...
	private int determinedStartingPlayer() {
		int max = cubeNumberList.stream().max(Comparator.naturalOrder()).get();
		...
		int zufaelligeZahl = new Random().nextInt(maxList.size());
		int zufaelligerIndex = maxList.get(zufaelligeZahl);
		JOptionPane.showMessageDialog(null,
				playerNameList.get(zufaelligerIndex) + " beginnt. Viel Spass.","Spielbeginn",
	 ...
	 */
	@FXML
	private void btnOnActionRollTheDice(MouseEvent event, int numberOfPlayers) {
		clickCounter++;
	...
	public void btnOnActionStartGame(ActionEvent event) {
		...
		int indexStartPlayer = determinedStartingPlayer();
 
		/*
		 * Initialisieren der Klasse Regel.
		 */
		Regel regel = new Regel();
		regel.erstelleSpieler(playerNameList, colorList, rivalList);
		...
		if((LauncherController.numberOfPlayers == 3) || (LauncherController.numberOfPlayers == 5) || (LauncherController.numberOfPlayers == 6)) {
			try {
				...
				gbsp.initBoard(regel, indexStartPlayer);
			...
			try {
				...
				gbfp.initBoard(regel, indexStartPlayer);
				Stage secondaryStage = new Stage();
